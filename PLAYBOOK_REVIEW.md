...
| 95 | Configure EVPN next-hop route-map | Preserves the original VTEP next hop when EVPN routes are re-advertised in eBGP. | `EG-BGP` |
| 101 | Configure leaf loopback redistribution route-map | Restricts underlay redistribution to tagged loopback routes instead of all connected routes. | `UG`, `EG-BGP` |
| 107 | Configure host SVI redistribution route-map | Restricts tenant VRF redistribution to host gateway SVIs only. | `EG-HostSVI`, `EG-BGP` |
| 113 | Configure base BGP on leafs | Instantiates the leaf BGP process and sets a deterministic router-id. | `UG`, `EG-BGP` |
| 119 | Configure leaf BGP underlay address-family | Enables ECMP and redistributes tagged loopbacks so overlay and VTEP addresses are reachable across the underlay. | `UG`, `EG-BGP` |
| 128 | Configure leaf BGP EVPN address-family | Enables the EVPN control plane and applies eBGP next-hop handling at the address-family level. | `EG-BGP` |
...
| 155 | Configure leaf EVPN neighbor address-family | Sends communities and applies the per-neighbor EVPN policy required by the eBGP overlay. | `EG-BGP` |
| 167 | Configure BGP VRF routing for leafs | Exports tenant IPv4 routes from the VRF RIB into EVPN so remote VTEPs can learn them. | `EG-VRF`, `EG-BGP` |
| 179 | Configure VLAN to VNI mappings | Binds each bridged tenant VLAN to its L2 VNI. | `EG-VLAN` |
| 186 | Configure SVIs for tenant gateways | Creates the distributed anycast default gateway for local endpoints in each VLAN. | `EG-HostSVI` |
...
| 279 | Configure EVPN next-hop route-map on spines | Preserves the originating leaf VTEP as next hop when the spine re-advertises EVPN routes. | `EG-BGP` |
| 285 | Configure spine loopback redistribution route-map | Restricts underlay redistribution to the tagged spine loopback only. | `UG`, `EG-BGP` |
| 291 | Configure base BGP on spines | Instantiates the spine BGP process and sets the router-id. | `UG`, `EG-BGP` |
| 297 | Configure spine BGP underlay address-family | Enables ECMP and redistributes the spine loopback into the eBGP underlay. | `UG`, `EG-BGP` |
| 306 | Configure spine BGP EVPN address-family | Enables EVPN and retains all RTs so the spine can relay EVPN routes without owning local VNIs. | `EG-BGP` |
...
| 333 | Configure spine EVPN neighbor address-family | Sends communities and applies the EVPN next-hop policy required for eBGP route propagation. | `EG-BGP` |
